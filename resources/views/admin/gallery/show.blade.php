    ...
    <div class="side-app">
        ...
        <div class="breadcrumb-6">
            <ol class="breadcrumb1 mb-0">
                ...
                <li class="breadcrumb-item1 active text-muted"> Multiple Images</li>
            </ol>
        </div>
        <!-- PAGE-HEADER END -->
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered text-nowrap border-bottom w-100" id="responsive-datatable">
                                ...
                                <tbody>
                                    @foreach ($MultipleGallery->images as $image)
                                        <tr>
                                            <td>{{ ++$loop->index }}</td>
                                            <td>
                                            ...
                                            <td>{{ $MultipleGallery->title }}</td>
                                              <td>
                                                <label class="custom-switch">
                                                    <input type="checkbox" class="custom-switch-input change-status" data-id="{{ $image->id }}" {{ $image->status == 'active' ? 'checked' : '' }}>
...
<script>
    ...
    $('body').on('click', '.change-status', function(){
        ...
        $.ajax({
            url: "{{ route('admin.multiple-image.change-status') }}",
            method: 'PUT',
